...
import z from "zod";
import { makeRateLimitPreHandler } from "../middleware/rateLimit";
...
export async function registerLinkRoutes(
  app: FastifyInstance,
  deps: { ipHashSalt: string; linkUseCase: CreateLinkUseCase; rateLimiter: RateLimiter | null },
) {
  const ipHasher = (ip: string) => hashIp(ip, deps.ipHashSalt);

  const rateLimitPreHandler = makeRateLimitPreHandler({
    rateLimiter: deps.rateLimiter,
    ipHasher,
    config: { limit: 20, windowSeconds: 60, keyPrefix: "create-link" },
  });
